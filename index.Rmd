...
## Data Exploration
The aim of exploration is to understand the data, reduce irrelevant or redundant features and select the most appropriate features for the models.
The paper describes the data set. The exercises were performed over approx 2 minutes with each user doing 10 repetitions of each "classe" in sequence from A to E. The sensors had a sampling rate of 45Hz meaning approx 45 samples were taken per second. Comparing the description given in the paper to what I see in the training set my understanding of the data is that the 160 variables are composed of:
...
1. The id and timestamp columns would bias the model and are not relevant to our prediction task.
2. The derived features are mostly 98% NA and would not help in prediction. Also we are not predicting from window summaries but from discrete samples, using the raw measurements.
...
### Visualisation
Let's look at these remaining 52 predictors, looking for potential outliers, skewness, imbalances in outcome/predictors, and explainable groups of points/patterns.
...
There are none so we can proceed with evaluating some models by splitting the cleaned training set into training and validation sets and then validating and tuning the models based on their performance on the validation set.
...
The first 2 (Bayes and KNN) are the simplest approaches. KNN requires some transformation of the data (standardising mean=0, stddev=1). The last 4 are tree-based. In order to get some perspective on the more complex final 3 tree ensemble methods I modeled a single decision tree to illustrate what these more complex algorithms are based on. It's important to understand the tree to understand the forest.

For cross-validation I will use 5-Fold Cross Validation to train the models and then estimate the out of sample error using the accuracy measure on the validation set. I used 5-Fold because rather than the default 10 in order to speed up training.
...
## Model Evaluation
The confusionMatrix function in caret helps to understand how the model would perform out of sample:
...
```{r echo=FALSE, warning=F, message=F, fig.width=12, fig.height=7, echo=F  }
  #layout(t(1:2))
  #plot( varImp(treeFit), main="CART Var Importance" )
  #plot( varImp(rfFit), main="Random Forest Var Importance" )
  #plot( varImp(gbmFit), main="GBM Var Importance" )
  #plot( varImp(xgbFit), main="XGB Var Importance" )
  
  # Plot compared var importance
  l1 <- varImp(treeFit, scale=F)$importance
  l2 <- varImp(rfFit, scale=F)$importance
  l3 <- varImp(gbmFit, scale=F)$importance
  l4 <- varImp(xgbFit, scale=F)$importance
...
### Accuracy
The accuracy graphs visualise the tuning parameters selected during cross-validation for the best fit. Models can be tuned with different parameters to see if accuracy improves.

```{r cache=F, warning=F, message=F, fig.width=4.5, fig.height=6, echo=F  }
  layout(t(1:2))
  ...
  #plot(xgbFit, main="XGB Accuracy")
  
  #names( xgbFit$results )
